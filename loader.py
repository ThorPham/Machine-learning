...
import itertools
import editdistance
#from dataHandler.preprocessing import preprocessing
...
def processing_input_ocr(path_image):
    image = Image.open(path_image)
    image_array = np.array(image)
    h,w = image_array.shape[:2]
    if (w < 120):
        padding_w = int((width - w))
        padding_h = int((height-h)/2)
        padding_size = (0,padding_h,padding_w,padding_h)
        #top_left,bot_left,top_right,bot_right = img_array[0,0],img_array[0,w-1],img_array[h-1,0],img_array[h-1,w-1]
        #fill_padd = tuple(np.mean([top_left,bot_left,top_right,bot_right],axis=0,dtype="int32"))
        
        image_padding = ImageOps.expand(image, padding_size,fill=tuple(image_array[0,0]))
        image_final = image_padding.resize((width,height),Image.NEAREST)
    else :
        image_final = image.resize((width,height),Image.NEAREST)
    return image_final
...
class VizCallback(keras.callbacks.Callback):
    def __init__(self, sess, y_func, text_img_gen, text_size, num_display_words=30):
        self.y_func = y_func
    ...
    def show_edit_distance(self, num):
        ...
        while num_left > 0:
            ...
            #print(decoded_res,"--",labels)
            for pre,ac in zip(decoded_res,labels):
                # print("predict :",len(pre))
                # print("------------------------------------")
                # print("actual :",len(ac))
                # print("------------------------------------")
                if pre.strip() == ac.strip() :
                    acc += 1
...
class TextImageGenerator:
    def __init__(self, img_dirpath, labels_path, img_w, img_h,
        ...
        # self.image_path = np.array(glob.glob(ps.pat.join(img_dirpath,"*png")))
        ...
        self.texts = []
        self.image_to_labels = [self.labels[label] for label in self.img_dir]
    ...
    def next_sample(self):
        ...
        return self.img_dir[self.indexes[self.cur_index]], self.image_to_labels[self.indexes[self.cur_index]]
    ...
    def next_batch(self):
        while True:
            ...
            input_length = np.ones((self.batch_size, 1), dtype=np.float32) * (
                        self.img_w // self.downsample_factor)  # (bs, 1)
            label_length = np.zeros((self.batch_size, 1), dtype=np.float32)  # (bs, 1)

            for i in range(self.batch_size):
                img_path, text = self.next_sample()
                #print(imgs)
                #print(os.path.join(self.img_dirpath, imgs))

                #img = image.load_img(os.path.join(self.img_dirpath, imgs), target_size=SIZE[::-1])
                #img = image.img_to_array(img)
                #img = preprocess_input(img).astype(np.float16)
                #img = img.transpose((1, 0, 2))
                image = processing_input_ocr(os.path.join(self.img_dirpath, img_path))
